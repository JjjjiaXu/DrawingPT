...
| semantic weighted scratch：inverse-sqrt | 1411 | foreground accuracy 0.251176；macro F1 0.020923 | macro F1 略升，预测更分散，但还没解决长尾 |
| semantic weighted pretrained：inverse-sqrt | 1412 | foreground accuracy 0.293149；macro F1 0.014104 | 没有优于 weighted scratch |

这组结果可以在组会上诚实地讲成：

> 现在链路已经不是“想法能不能实现”的问题，而是进入了“训练目标和数据分布会不会把模型带偏”的问题。class 0 放进 loss 会造成 overall accuracy 虚高，但前景 F1 为 0；改成 foreground-only loss 后，模型开始命中前景，但 100 step / 1% 标注下主要猜 wall 和 sink。inverse-sqrt class weighting 能让预测更分散一点，macro F1 从 0.0168 到 0.0209，但还没解决 rare class。所以下一步要做 class-aware window sampling，再扩大 label-efficiency 实验。

## 8. 下周真正要过的门禁

| 门禁 | 通过标准 |
|---|---|
| 类别均衡门禁 | semantic fine-tuning 加 class-aware window sampling / focal loss，foreground macro F1 明显高于当前 best smoke 0.0209 |
